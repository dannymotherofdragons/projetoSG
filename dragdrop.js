...
function mouseDown(event) {
    ...
    /*if (intersects[0].object.indexOf(fruits) != intersects[0].object.indexOf(fruits)) {
        timeClicked = 1;
...
/*function doubleClick(event){
    ...
    console.log("This is haoengubasifbj")
    let mousePoint = new THREE.Vector2();
    mousePoint.x = (event.clientX / window.innerWidth) * 2 - 1;
    mousePoint.y = -(event.clientY / window.innerHeight) * 2 + 1;
...
function animate() {
    ...
    if (keyboard[87]) { //s key
        ...
        plane.position.x -= Math.sin(camera.rotation.y) * player.speed
        plane.position.z -= Math.cos(camera.rotation.y) * player.speed
    }
